...
class ToolErrorsTracking:
    """
        Simple class to track errors in tools during Task execution. There's 2 type of errors that can occur inside a tool.
        When a tool signals an error then we decrement the error counter which when reaching 0 triggers an exception.
    """
    ...
    def log_new_error(self, tool: Tool, error_type: Status):
        if error_type == Status.CALL_ERROR:
            ...
            self.session_status[tool.tool_name].custom_error -= 1

        if self.session_status[tool.tool_name].call_error < 0:
        ...
        if self.session_status[tool.tool_name].custom_error < 0:
            raise MaxToolErrorIter(
        ...
        while True:
            ...
            if isinstance(self.agent.history.get_last_message(), OpenAIFunctionCallingMessage):
                for tool_call in self.agent.history.get_last_message().tool_calls:
                    ...
                    tool_output, status = self._call_openai_tool(tool, func_arguments)
    ...
    def _call_openai_tool(self, tool: Tool, function_args: Dict) -> (str, Status):
        ...
        try:
            ...
            if type(e) is ToolError or type(e) is JSONDecodeError:
                ...
                tool_output = str(e)
                status = Status.CALL_ERROR
            return tool_output, status
        return tool_output, Status.OKAY
