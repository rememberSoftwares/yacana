...
class Tool:
    ...
    def __init__(self, tool_name: str, function_description: str, function_ref: Callable, optional: bool = False,
        ...
        :param tool_name: str: A name for the tool. Should be concise and related to what the tool does.
        :param function_description: str: A description for the tool. Should be concise and related to what the tool does. May contain an example of how to use. Refer to the documentation.
        :param function_ref: Callable: The reference to a python function that will be called with parameters provided by the LLM.
        :param optional: bool: Allows to a certain extent the LLM to choose to use the given tool or not depending on the task to solve.
        :param usage_examples: List[dict]: A list of python dictionary examples of how the tool should be called. The examples will be given to the LLM to help it call the tool correctly. Use if the LLM struggles to call the tool successfully.
        :param max_custom_error: int: The max errors a tool can raise. A tool should raise a ToolError(...) exception with a detailed explanation of why it failed. The LLM will get the exception message and try again, taking into account the new knowledge it gained from the error. When reaching the max iteration the MaxToolErrorIter() exception is thrown and the task is stopped.
        :param max_call_error: int: The max number of times Yacana can fail to call a tool correctly. Note that Yacana uses the parameters given to the LLM to call the tool so if they are invalid then Yacana will have a hard time to fix the situation. You should try to give examples to the LLM on how to call the tool either in the tool description or when using the @usage_examples attribute to help the model.
        """
